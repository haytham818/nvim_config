# Neovim 键位映射文档 (Keymaps Documentation)

本文档包含 Neovim 的默认键位、自定义键位以及所有已安装插件的键位映射。
...
1. **文本对象组合**: `d` (删除) + `i` (内部) + `w` (单词) = `diw` (删除单词)
2. **重复操作**: 使用数字前缀，如 `3dd` (删除 3 行)
3. **范围选择**: `v` 进入可视模式 → 移动选择 → 操作 (如 `d` 删除)
4. **宏的使用**: `qa` (开始录制) → 执行操作 → `q` (停止) → `@a` (执行)

### 效率提升建议

1. **学习动作**: 熟悉 `w`, `b`, `e`, `f`, `t` 等动作键
2. **使用文本对象**: `ci"` (修改引号内容) 比手动选择快
3. **利用搜索**: `*` 快速搜索当前单词
4. **使用标记**: 在重要位置设置标记，快速跳转
5. **掌握 . 键**: 重复上次操作，非常高效
...
本配置结合了 Neovim 的强大默认功能和现代插件的便利性，旨在提供高效的编辑体验。建议：

1. 先熟悉 Neovim 的基本键位
2. 逐步学习插件的特殊功能
3. 使用 Which-Key (`<leader>?`) 查看可用键位
4. 根据个人习惯调整配置

**配置文件位置**:
- 主配置: `init.lua`
- 键位映射: `lua/config/keymaps.lua`
- 插件配置: `lua/plugin/`
- LSP 配置: `lua/lsp.lua`
