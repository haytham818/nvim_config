...
- [插件键位 (Plugin Keymaps)](#插件键位-plugin-keymaps)
  - [文件管理器 (mini.files)](#文件管理器-minifiles)
...
| 键位         | 模式   | 功能                                         |
| ------------ | ------ | -------------------------------------------- |
| `<leader>e`  | Normal | 打开当前文件所在目录并启用预览               |
| `-`          | Normal | 切换当前文件所在目录；在管理器内返回上级     |
| `<leader>cw` | Normal | 在工作目录打开/关闭文件管理器                |

**mini.files 内部键位**:

- `l` / `<CR>` - 打开文件并自动关闭面板；进入目录时保持面板打开
- `L` - 打开文件或进入目录，但保留面板
- `h` - 返回父目录
- `<Esc>` / `q` / `<C-c>` - 关闭面板
- `<C-s>` - 垂直分屏打开文件并关闭面板
...
- 行尾会显示 Git 状态与 LSP 诊断摘要
- Git: `A` 新增, `M` 修改, `R` 重命名, `D` 删除, `?` 未跟踪, `!` 已忽略
- LSP: `E/W/I/H` 分别表示 Error / Warn / Info / Hint，后跟数量
